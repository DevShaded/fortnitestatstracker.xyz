...
class FortniteController extends Controller
{
    ...
    private function updatePlayerInDB(string $username)
    {
        ...
        if ($response['status'] == 200) {
            ...
            $this->updateLifetimeStatsToDB($playerID, $response);
            $this->updateKeyboardStatsToDB($playerID, $response);
            $this->updateGamepadStatsToDB($playerID, $response);
        }
    }
    ...
    /**
     * Here we update the lifetime stats with new stats
     *
     * @param $playerID
     * @param $response
     * @return void
     */
    private function updateLifetimeStatsToDB($playerID, $response)
    {
        $currentTime = date('Y-m-d H:i:s');
    ...
    /**
     * Here we update the keyboard & mouse stats with new stats
     *
     * @param $playerID
     * @param $response
     * @return void
     */
    private function updateKeyboardStatsToDB($playerID, $response)
    {
        $currentTime = date('Y-m-d H:i:s');
    ...
    /**
     * Here we update the gamepad stats with new stats
     *
     * @param $playerID
     * @param $response
     * @return void
     */
    private function updateGamepadStatsToDB($playerID, $response)
    {
        $currentTime = date('Y-m-d H:i:s');
